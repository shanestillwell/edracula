require("plenary.reload").reload_module("edracula", true)
local edracula = require("edracula")
local default = edracula.config
...
describe("tests", function()
  it("works with default values", function()
    edracula.setup()
    assert.are.same(edracula.config, default)
  end)
  ...
  it("works with config overrides", function()
    ...
    edracula.setup({ undercurl = false, underline = false })
    assert.are.same(edracula.config, expected)
  end)
  ...
  it("does not set terminal colors when terminal_colors is false", function()
    clear_term_colors()
    edracula.setup({ terminal_colors = false })
    edracula.load()
  ...
  it("sets terminal colors when terminal_colors is true", function()
    clear_term_colors()
    edracula.setup({ terminal_colors = true })
    edracula.load()

    -- dark bg
    local colors = require("edracula").palette
